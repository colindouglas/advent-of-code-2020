...
next_cup <- vector(mode = "integer", length = total_cups)

for (i in seq_along(circle)) {
  next_cup[circle[i]] <- as.integer(circle[i + 1])
}

next_cup[length(next_cup)] <- 1  # Loop around to the start
...
play_game <- function(n) {
  ...
  for (nothing in seq_along(n)) {
    ...
    while(dest_cup %in% c(cup1, cup2, cup2)) {
      dest_cup <- current_cup - 1
      if (dest_cup < 1) dest_cup <- total_cups
    }
    
    # The crab places the cups it just picked up so that they are immediately clockwise of the destination cup. 
    # They keep the same order as when they were picked up.
    dest_cup_next <- next_cup[dest_cup]
    next_cup[dest_cup] <- cup1
    next_cup[cup3] <- dest_cup_next
    
    # The crab selects a new current cup: the cup which is immediately clockwise of the current cup.
    current_cup <- next_cup[current_cup]
  }
  
  next_cup[1] * next_cup[2]
}
